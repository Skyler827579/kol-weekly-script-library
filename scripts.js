...
function getCavenScript(script, primary, secondary, market, secondMarket) {
  ...
  if (script.week === 0) {
    return [
      ...
      `Let me give you a simple example. Imagine SOL has been ranging between 95 and 98 for a few hours. Everyone can see the low at 95. That level becomes obvious. When a level is obvious, stop losses collect there. Short sellers also wait for a breakdown there. So if price suddenly dips under 95, a lot of people react at the same time.`,
      `But the key question is: does price stay below 95, or does it snap back above? If price breaks below 95 and immediately reclaims it, that tells us the breakdown may be a trap. The market took liquidity from late sellers and stop losses, then returned back into the range. That is the first clue.`,
  ...
  if (script.week === 2) {
    return [
      ...
      `Here is an example. If SOL is moving only one dollar on average in the current session, but your target is four dollars away, that target is not realistic for a scalp. You may be right on direction and still fail because your expectation does not match volatility.`,
...
function getKcScript(script, primary, secondary, market, secondMarket) {
  ...
  if (script.week === 0) {
    return [
      ...
      `Let me give you a simple example. Suppose BTC is trading above the 20 EMA, then it pulls back from 82,000 to the EMA area around 81,000. A beginner sees red candles and gets scared. A trader asks a better question: is this a normal pullback, or is the trend breaking? If RSI stays near the middle and price holds the EMA, that is still a healthy pullback.`,
  ...
  if (script.week === 2) {
    return [
      ...
      `Example: if SOL pulls back to the 20 EMA and ATR is around 1.7 dollars, a realistic first target might be around one ATR away, not five or six dollars away. If BTC ATR is much larger, the BTC target can be wider. Different asset, different volatility, different target.`,
  ...
  return [
    ...
    `Example: BTC pushes from 78,000 to 82,000. You draw Fibonacci on that move. If the 0.5 pullback is near 80,000 and the 20 EMA is also near 80,000, that becomes your attention zone. Not automatic entry. Attention zone.`,
